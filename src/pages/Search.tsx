...
export const Search: React.FC = () => {
 ...
 className="w-full bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 font-sans pl-11 pr-4 py-3.5 rounded-2xl border border-zinc-200 dark:border-zinc-800 focus:border-zinc-950 dark:focus:border-zinc-100 focus:outline-none focus:ring-1 focus:ring-zinc-950/10 dark:focus:ring-white/10 placeholder-zinc-400 dark:placeholder-zinc-500 text-sm shadow-xs transition-all font-medium"
 placeholder="Search by keywords, display names, bios, usernames, or tag labels..."
 />
 </div>

 {/* Tabs selectors representation */}
 <div className="flex mb-6 gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-1 rounded-2xl w-fit xl:w-full">
 <button
 id="btn-search-tab-users"
 onClick={() => setActiveTab('users')}
 className={`py-2 px-4 rounded-xl font-semibold text-sm transition-all focus:outline-none cursor-pointer ${
 activeTab === 'users'
 ...
 <p className="text-xs text-zinc-400 dark:text-zinc-500 italic">Try searching with a different user nickname or display tag</p>
 </div>
 ) : (
 ...
 return (
 <div key={u.id} className="bg-white dark:bg-zinc-900 hover:bg-zinc-100/30 dark:hover:bg-zinc-800/40 transition-all border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 flex flex-col justify-between shadow-xs gap-3">
 ...
 <p className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 truncate">@{u.username}</p>
 {u.bio && <p className="text-xs text-zinc-600 dark:text-zinc-300 line-clamp-2 mt-1.5 leading-relaxed">{u.bio}</p>}
 </div>
 </div>

 <div className="flex gap-2 pt-3 mt-1.5 justify-end">
 <button
 ...
 isFollowing
 ? 'border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 hover:bg-red-50 dark:hover:bg-red-950/20 hover:text-red-600 dark:hover:text-red-400 hover:border-red-200'
 ...
 <p className="text-xs text-zinc-400 dark:text-zinc-500 italic">Try searching hashtag topics like #minimalist or keyword triggers</p>
 </div>
 ) : (
 <div className="flex flex-col gap-6">

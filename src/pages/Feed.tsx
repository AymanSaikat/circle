...
export const Feed: React.FC<FeedProps> = ({ selectedTag, setSelectedTag, currentTab }) => {
 ...
 className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 rounded-xl pl-11 pr-4 py-3 text-xs focus:outline-none focus:border-zinc-900 dark:focus:border-zinc-100 focus:ring-1 focus:ring-zinc-900/10 dark:focus:ring-white/10 transition-all shadow-xs"
 />
 {searchQuery && (
 <button
 id="btn-clear-search"
 onClick={() => setSearchQuery('')}
 className="absolute inset-y-0 right-0 pr-4 flex items-center text-xs text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white font-bold cursor-pointer"
 >
 Clear
 </button>
 )}
 ...
 <div ref={observerTarget} className="py-8 flex flex-col items-center justify-center mt-6 select-none">
 {loadingMore ? (
 ...
 <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono tracking-wider select-none text-center">
 ...
 <div className="mt-6 pt-5 select-none">
 ...
 <div className="text-[10px] text-zinc-400 dark:text-zinc-500 truncate select-text">
 @{sug.username}
 </div>

 {/* Display connection context naturally */}
 <div className="text-[9px] font-mono text-zinc-400 dark:text-zinc-500 truncate mt-0.5 select-text">
 {sug.mutualCount > 0 ? (
 ...
 <span className="text-zinc-400 dark:text-zinc-500">Suggested for you</span>
 )}
 </div>
 </div>

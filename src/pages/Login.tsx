...
export const Login: React.FC = () => {
  const { signInWithGoogle, signUpWithEmail, signInWithEmail, checkUsernameUnique } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-6 relative overflow-hidden select-none">
      {/* Decorative subtle blurs */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-zinc-200/40 dark:bg-zinc-800/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-zinc-200/30 dark:bg-zinc-800/10 rounded-full blur-3xl pointer-events-none" />

      {/* Main card box */}
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-zinc-205 dark:border-zinc-800 rounded-2xl p-8 shadow-xs relative z-10 transition-all">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {error && (
            <div className="bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/40 p-3 rounded-xl text-[11px] text-red-650 dark:text-red-400 font-mono font-bold">
              {error}
            </div>
          )}
      ...
      {/* Humble Footer info */}
      <div className="mt-8 flex items-center gap-1 text-[11px] text-zinc-400 dark:text-zinc-500 font-medium font-sans select-none">
        <span>Circle • Minimal Media Platform</span>
      </div>
    </div>
  );
};
